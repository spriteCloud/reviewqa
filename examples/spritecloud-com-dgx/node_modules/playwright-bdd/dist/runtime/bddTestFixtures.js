...
const promptAttachment_1 = require("../ai/promptAttachment");
...
exports.test = bddWorkerFixtures_1.test.extend({
    ...
    Given: [
        async ({ $bddContext, $world, $applySpecialTags }, use) => {
            const invoker = new bddStepInvoker_1.BddStepInvoker($bddContext, $world);
    ...
    $runScenarioHooks: [
        async ({ $bddContext, $world }, use) => {
            await use(async (type, customFixtures) => {
                const hooksToRun = (0, scenario_1.getScenarioHooksToRun)(type, $bddContext.tags);
                if (type === 'after')
                    hooksToRun.reverse();
                await (0, scenario_1.runScenarioHooks)(hooksToRun, $world, { $bddContext, ...customFixtures });
            });
        },
        fixtureOptions,
    ],
    ...
    $prompt: [
        async ({ $bddContext }, use) => {
            const prompt = new promptAttachment_1.PromptFixture($bddContext);
            await use(prompt);
            await prompt.attach();
        },
        ...
        supportedFeatures_1.supportedFeatures.boxedFixturesWarning ? { scope: 'test' } : fixtureOptions,
    ],
});
