 ...
 *
 * Accessibility landmark structure for https://playwright.dev/agent-cli/introduction. v0.59 — depth
...
test.describe('PlaywrightDev accessibility landmarks @ agent-cli/introduction page', () => {
  test('@kind:a11y-landmarks @smoke one main region, one h1, at least one nav', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks @negative no interactive elements inside aria-hidden', async ({ page }) => {
    await page.goto('/agent-cli/introduction')
  ...
  test('@kind:a11y-landmarks @heading-hierarchy headings follow sequential level order', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks @landmark-names multiple landmarks of same type have distinguishing names', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks @skip-link "skip to content" link or targeted anchor present (WCAG 2.4.1)', async ({ page }) => {
