 ...
 *
 * Accessibility landmark structure for https://playwright.dev/agent-cli/configuration. v0.59 — depth
...
test.describe('PlaywrightDev — a11y landmarks @ https://playwright.dev/agent-cli/configuration', () => {
  test('@kind:a11y-landmarks @smoke verify single main, h1, and nav regions', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks @negative verify no focusable elements inside aria-hidden regions', async ({ page }) => {
    await page.goto('/agent-cli/configuration')
  ...
  test('@kind:a11y-landmarks @heading-hierarchy verify heading levels follow sequential order', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks @landmark-names verify duplicate landmarks have accessible names', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks @skip-link verify "skip to content" affordance present (WCAG 2.4.1)', async ({ page }) => {
