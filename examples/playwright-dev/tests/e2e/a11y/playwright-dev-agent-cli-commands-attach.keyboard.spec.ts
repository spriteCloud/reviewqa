 ...
 *
 * Keyboard-only navigation for https://playwright.dev/agent-cli/commands/attach. v0.59 — testing keyboard accessibility beyond static analysis:
 * five tests covering keyboard interactions axe-core cannot detect:
 *
 *   smoke           — Tab cycles through the first 10 focusable elements
 *   focus-indicator — the first element focused by Tab has a visible focus indicator
 *   escape-dismiss  — Escape closes an open dialog and returns focus to the trigger
 *   enter-space     — Enter activates links and buttons; Space activates buttons
 *   no-trap         — Tab from the last focusable wraps or exits (no trapping)
 *
 * Run with: `npx playwright test --grep @kind:keyboard`
 */
import { test, expect } from '@playwright/test'

test.describe.configure({ mode: 'parallel' })
test.describe('PlaywrightDev — keyboard navigation @ https://playwright.dev/agent-cli/commands/attach', () => {
  test('@kind:keyboard @smoke tabs through the first 10 focusable elements', async ({ page }) => {
    await page.goto('/agent-cli/commands/attach')
  ...
  test('@kind:keyboard @no-trap Tab from the last focusable wraps to the start or exits the page', async ({ page }) => {
