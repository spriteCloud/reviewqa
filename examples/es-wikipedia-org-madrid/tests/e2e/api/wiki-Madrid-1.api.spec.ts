 ...
 *   - this file (*.api.spec.ts): "POST https://es.wikipedia.org/wiki/Madrid still accepts our shape"
 *
 * Endpoint:     https://es.wikipedia.org/wiki/Madrid
 * Method:       post
...
test.describe('EsWikipedia — API contract @ https://es.wikipedia.org/wiki/Madrid', () => {

  test('@kind:api @smoke happy: realistic body returns 2xx', async ({ request }) => {
    const response = await request.post('https://es.wikipedia.org/wiki/Madrid', {
      form: {
  ...
  test('@kind:api @negative wrong method returns 4xx', async ({ request }) => {
    ...
    const response = await request.get('https://es.wikipedia.org/wiki/Madrid')
  ...
  test('@kind:api @negative rapid burst stays healthy or is rate-limited', async ({ request }) => {
    ...
    const responses = await Promise.all(
      Array.from({ length: 10 }, () => request.post('https://es.wikipedia.org/wiki/Madrid', { form: body }))
    )
