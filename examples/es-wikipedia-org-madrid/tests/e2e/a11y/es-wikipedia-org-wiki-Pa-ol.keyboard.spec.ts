 ...
 *
 * Keyboard-only navigation for https://es.wikipedia.org/wiki/Pa%C3%B1ol. v0.59 — depth parity:
...
test.describe('EsWikipediaOrg — keyboard navigation @ https://es.wikipedia.org/wiki/Pa%C3%B1ol', () => {
  test('@kind:keyboard @smoke tab through the first 10 focusables', async ({ page }) => {
    await page.goto('/wiki/Pañol')
