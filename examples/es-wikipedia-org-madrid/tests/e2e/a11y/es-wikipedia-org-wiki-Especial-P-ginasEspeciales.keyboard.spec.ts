 ...
 *
 * Keyboard-only navigation for https://es.wikipedia.org/wiki/Especial:P%C3%A1ginasEspeciales. v0.59 — depth parity:
...
test.describe('EsWikipediaOrg — keyboard navigation @ https://es.wikipedia.org/wiki/Especial:P%C3%A1ginasEspeciales', () => {
  test('@kind:keyboard @smoke tab through the first 10 focusables', async ({ page }) => {
    await page.goto('/wiki/Especial:PáginasEspeciales')
