 ...
 * Generated by quail.
 *
 * HTTP error / redirect chains baseline for https://www.ing.nl. Asserts the
...
test.describe('ING HTTP redirect and error chain handling', () => {
  test('handles multi-hop redirects (301 → 302 → 200)', async ({ request }) => {
  ...
  test('returns a user-friendly error for 410 (Gone) responses', async ({ request }) => {
  ...
  test('handles server-side rate-limit (429) with Retry-After header', async ({ context, page }) => {
    let firstCall = true
