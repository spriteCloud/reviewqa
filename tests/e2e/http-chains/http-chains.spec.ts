...
test.describe('Ing — HTTP redirect chains for https://www.ing.nl', () => {
  ...
  test('@kind:http-chain returns a friendly error for nonexistent paths (410 Gone)', async ({ request }) => {
  ...
  test('@kind:http-chain handles server-side rate-limit response shape (429 with Retry-After)', async ({ context, page }) => {
    let firstCall = true
