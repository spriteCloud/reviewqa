 ...
 * Generated by quail.
 *
 * Accessibility landmark structure for https://www.ing.nl/particulier. v0.59 — depth
 * parity. Five tests covering the structural a11y concerns the
 * baseline axe scan does NOT enforce:
 *
 *   smoke               — exactly one main region, one h1, and at least one nav
 *   no-aria-hidden      — no focusable elements inside aria-hidden regions
 *   heading-hierarchy   — headings follow sequential order (e.g., no skipping from h1 to h3)
 *   landmark-names      — multiple instances of key landmarks each have a unique accessible name
...
test.describe('Ing — a11y landmarks @ https://www.ing.nl/particulier', () => {
  test('@kind:a11y-landmarks smoke: single main, h1, and nav present', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks heading hierarchy: order is sequential', async ({ page }) => {
  ...
  test('@kind:a11y-landmarks landmark names: duplicates have accessible names', async ({ page }) => {
