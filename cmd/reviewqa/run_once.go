...
// newRunOnceCmd registers `reviewqa run-once`. Closes the v0.43d gap:
...
// into the ledger, and prepares the next `reviewqa generate` to emit
// sentinel specs.
//
...
the next ` + "`reviewqa generate`" + ` emits a sentinel spec per failing
test under tests/e2e/sentinels/.

Use this once after the first ` + "`reviewqa probe`" + ` to bootstrap the
...
func runOnce(workDir string, record bool, reportPath, grep string) error {
	...
	if _, err := os.Stat(e2eDir); err != nil {
		return fmt.Errorf("run-once: no tests/e2e/ under %s — run `reviewqa probe` first", workDir)
	}
...
func mergeReportIntoLedger(workDir, reportPath string) error {
	...
	rlog.Info("run-once: re-run `reviewqa generate` (or probe) to emit sentinels for each finding")
	return nil
}
