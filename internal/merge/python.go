...
var pyImportRE = regexp.MustCompile(`(?m)^(?:from\s+\S+\s+import\s+.+|import\s+.+)$`)

func appendPython(existing, generated []byte) ([]byte, bool) {
	oldImports, oldRest := splitPyImports(existing)
	newImports, newRest := splitPyImports(generated)
...
func splitPyImports(src []byte) (imports []string, rest string) {
	matches := pyImportRE.FindAllIndex(src, -1)
	if len(matches) == 0 {
